  ...
  async function find_Mail_And_Resgister() {
    ...
    if (userResult.data && userResult.data.length > 0) {
      ...
      if (mobile_no.length == 10) {
        ...
        main();
      } else { res.send("mobile no is invlid") }
...
app.post("/api/uplode_image", upload.single('file'), async function (req, res) {
  if (!req.file) {
...
app.post("/api/add_jatra", async function (req, res) {
  ...
  const { data, error } = await supabase
    .from('jatra')
    .insert([{ title: title, description: description, poster: poster, ticket_price: ticket_price, place: place, city: city, date: date, time: time }]);
  if (!error) {
...
app.post("/api/no_of_seats/jatra_id", async function (req, res) {
  const jatra_id = req.body.jatra_id;
  const supabase = await supabasePromise;
  const { data, error } = await supabase
    .from('booked_seats')
...
app.post("/api/search", async function (req, res) {
  ...
  try {
    ...
    if (uniqueData.length == 0) {
      ...
      res.json({ message: 'found', data: uniqueData });
    }
  } catch (error) {
    console.error(error);
    res.status(500).send('Error creating order');
